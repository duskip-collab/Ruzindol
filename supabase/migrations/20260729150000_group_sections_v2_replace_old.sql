-- V2 consolidation migration for community group sections.
-- Safely replaces/normalizes previous group migrations.
...
-- Replace old group_key constraints with a single updated set.
DO $$
DECLARE
  r record;
BEGIN
  IF to_regclass('public.group_admins') IS NOT NULL THEN
    FOR r IN
      SELECT c.conname
