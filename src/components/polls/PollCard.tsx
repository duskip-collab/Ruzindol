...
export const PollCard: React.FC<{
  ...
  onVoteSuccess?: () => void;
  className?: string;
}> = ({ poll, isActiveNeighbor = true, onVoteSuccess, className }) => {
  const [selectedOptionId, setSelectedOptionId] = useState<string | null>(
    poll.user_voted_option_id || null,
  );
  ...
  const handleVote = async () => {
    if (!selectedOptionId) {
      triggerHaptic("error");
      setErrorMessage("Vyberte možnosť.");
      return;
    }
    if (!isActiveNeighbor) {
      triggerHaptic("error");
      setErrorMessage("Hlasovať môžu iba overení susedia.");
      return;
    }
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-white",
        className,
      )}
    >
        ...
        {poll.options.map((opt) => {
          ...
          if (hasVoted || isExpired) {
            return (
              <div
                key={opt.id}
                className={cn(
                  "relative overflow-hidden rounded-xl border p-2 text-xs",
                  isSel
              ...
              >
                ...
                <div className="relative z-10 flex items-center justify-between font-semibold">
                  <div className="flex items-center gap-1.5">
                    {isSel && <CheckCircle2 className="h-4 w-4 text-blue-600" />}
          ...
          return (
            <button
              ...
              className={cn(
                "flex w-full items-center justify-between rounded-xl border p-2 text-xs font-semibold",
                isSel
