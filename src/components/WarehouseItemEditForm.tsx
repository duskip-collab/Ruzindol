    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          price: item.type === "darovanie" ? 0 : Number(price) || 0,
          ...fields,
        })
        .eq("id", item.id)
      ...
      if (storageResults.some(Boolean)) {
        setError(
          "Inzerát bol uložený, ale niektoré staré fotky sa nepodarilo odstrániť zo storage.",
        );
      }
      ...
      if (!databaseSaved) {
        await Promise.all(
          uploaded.map((photo) =>
            removeBucketObject("warehouse", photo.path).catch(() => undefined),
          ),
        );
      }
    ...
    <div className="fixed inset-0 z-[180] flex items-end bg-black/40 p-0 backdrop-blur-sm md:items-center md:justify-center md:p-5">
      <div className="flex h-full w-full flex-col bg-background text-foreground md:h-auto md:max-h-[92%] md:max-w-2xl md:rounded-3xl md:border md:border-border md:shadow-2xl">
        ...
        <form
          ...
          className="flex flex-1 flex-col gap-4 overflow-y-auto p-5 pb-safe"
        >
          ...
          <div>
            <p className="text-sm font-medium">
              Fotografie ({photoCount}/{MAX_PHOTOS})
            </p>
          ...
          {saved && (
            <p className="text-sm font-medium text-emerald-600">Zmeny boli úspešne uložené.</p>
          )}
          <div className="mt-auto flex gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="btn-secondary-surface flex-1 px-4 py-3 text-sm font-medium"
            >
              Zrušiť
            </button>
            <button
              ...
              className="btn-primary-glow flex flex-1 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold disabled:opacity-60"
            >
              {busy && <Loader2 className="h-4 w-4 animate-spin" />} Uložiť zmeny
            </button>
          </div>
        </form>
      </div>
